...
class qatAccess(Poll, Converter):
	CAID = 0
	PID = 1
	BETACAS = 2
	IRDCAS = 3
					...
					if ecm_info:
						...
						if self.type == self.ECMINFO:
							if "fta" in protocol:
								...
								if source == "emu":
									ecminfo = "CA: %s:%s %s PID:%s  Source: %s@%s " % (caid, prov,ecm_time, pid, source, frm)
								elif reader != "" and source == "net" and port != "":
									ecminfo = "CA: %s:%s %s PID:%s %s@%s %s %s (%s) Source: %s:%s %s" % (caid, prov,ecm_time, pid, reader, system, frm, protocol, source, server, port, hopsr)
								elif reader != "" and source == "net" and not "fta" in protocol:
									ecminfo = "%s@%s CA: %s:%s %s PID:%s %s %s (%s) Source: %s %s" % (reader, frm, caid, prov,ecm_time, pid, system, protocol, source, server, hops)
								elif reader != "" and source != "net":
									ecminfo = "%s@%s CA: %s:%s  Ecm Time: %s PID:%s %s %s (local) - %s %s" % (reader, frm, caid, prov,ecm_time, pid, system, protocol, source, hops)
								elif server == "" and port == "" and protocol != "":
									ecminfo = "CA: %s:%s  Ecm Time: %s PID:%s  Prtc: %s (%s) %s" % (caid, prov, pid, protocol, source, hops, ecm_time)
								elif server == "" and port == "" and protocol == "":
									ecminfo = "CA: %s:%s  Ecm Time: %s PID:%s  Source: %s" % (caid, prov, ecm_time, pid, source)
								else:
									try:
										ecminfo = "%s:%s  %s Ca: %s:%s  pid:%s  %s %s (%s) %s" % (server, port, ecm_time, caid, prov,  pid, system, protocol, source, hops)
									except:
										pass
							else:
								ecminfo = casi
